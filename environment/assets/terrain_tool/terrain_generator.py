...
class TerrainGenerator:
    ...
    def __init__(self) -> None:
        self.scene = xml_et.parse(INPUT_SCENE_FILE)
    ...
    def AddStairs(self, init_pos=[1.0, 0.0, 0.0], yaw=0.0, width=0.2, height=0.15, length=1.5, stair_nums=10):
        ...
        for i in range(stair_nums):
            ...
            self.AddBox([x + init_pos[0], y + init_pos[1], local_pos[2]], [0.0, 0.0, yaw], [width, length, height])
    ...
    def AddSuspendStairs(self,
        ...
        for i in range(stair_nums):
            ...
            local_pos[2] += height
            x, y = rot2d(local_pos[0], local_pos[1], yaw)
            self.AddBox([x + init_pos[0], y + init_pos[1], local_pos[2]], [0.0, 0.0, yaw],
    ...
    def AddPerlinHeighField(
            ...
            output_hfield_image="height_field"):

        output_hfield_image_ = f"{output_hfield_image}_{idx}.png"
        ...
        hfield.attrib["file"] = "../" + output_hfield_image_

        geo = xml_et.SubElement(self.worldbody, "geom")
        geo.attrib["type"] = "hfield"
        geo.attrib["hfield"] = f"perlin_hfield_{idx}"
        geo.attrib["pos"] = list_to_str(position)
        quat = euler_to_quat(euler[0], euler[1], euler[2])
        geo.attrib["quat"] = list_to_str(quat)
    ...
    def AddHeighFieldFromImage(
        ...
        output_hfield_image_ = f"{output_hfield_image}_{idx}"
        ...
        cv2.imwrite("../unitree_robots/" + ROBOT + "/" + output_hfield_image_ + {idx}, terrain_image)
    ...
    def Save(self):
        self.scene.write(OUTPUT_SCENE_FILE)
...
if __name__ == "__main__":
    ...
    # # Perlin heigh field
    # tg.AddPerlinHeighField(position=[3.5, 0, 0.0], size=[wide, wide])
