...
def evaluate(agent: QDecompSAC, env: Go2Env, n_episodes: int = 5) -> dict:
    ...
    for _ in range(n_episodes):
        ...
        # Success: robot reached upright (orientation reward > 0.8)
...
def train(total_timesteps_override: int | None = None):
    ...
    for t in range(1, total_timesteps + 1):
        ...
        if done:
            ...
            # Log per-subagent running reward contributions
            for key in REWARD_KEYS:
                val = reward_breakdown.get(key, 0.0)
                writer.add_scalar(f'train/reward_{key}', val, t)
            ...
            if episode_num % 10 == 0:
                print(f"  Step {t:>8,} | Episode {episode_num:>5} | "
