...
def _mlp(input_dim: int, hidden_sizes: list[int], output_dim: int) -> nn.Sequential:
    """MLP with ReLU activations; last layer is a Linear with no activation."""
...
def _trunk(input_dim: int, hidden_sizes: list[int]) -> nn.Sequential:
    """Shared feature trunk (all layers with ReLU, no output projection)."""
    layers: list[nn.Module] = []
    in_dim = input_dim
    for h in hidden_sizes:
        layers += [nn.Linear(in_dim, h), nn.ReLU()]
...
class QDecompCritic(nn.Module):
    ...
    def forward(
        ...
        """Returns lists of Q1_j and Q2_j values, one per subagent."""
